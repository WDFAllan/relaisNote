...
function App() {
  ...
  useEffect(() => {
    if (!token) return
    Promise.all([loadBeneficiaries(), api<Tag[]>('/api/tags', { headers: authHeaders }).then(setTags)]).catch((e) => setError(e.message))
  }, [token])
  ...
  return <main className="app-shell"><header className="topbar"><div className="wordmark"><span>R</span><strong>relais</strong></div><div className="topbar-right"><span className="status-dot">● API connectée</span><span>{user?.prenom} {user?.nom}</span><button className="quiet-button" onClick={logout}>Quitter</button></div></header><section className="workspace"><aside className="sidebar"><div className="section-heading"><div><p className="eyebrow">SUIVI ACTUEL</p><h2>Bénéficiaires</h2></div><button className="icon-button" onClick={() => setShowCreate(true)} title="Ajouter un bénéficiaire">+</button></div><div className="search-box">⌕ <input placeholder="Rechercher..." /></div><div className="people-list">{beneficiaries.map((item) => <button className={`person-row ${selectedId === item.id ? 'selected' : ''}`} key={item.id} onClick={() => setSelectedId(item.id)}><span className="avatar">{item.prenom[0]}</span><span><strong>{item.prenom}</strong><small>{item.referentNom ?? 'Sans référent'}</small></span><span className="chevron">›</span></button>)}{beneficiaries.length === 0 && <p className="empty">Aucun bénéficiaire actif.</p>}</div></aside><section className="content"><div className="content-heading"><div><p className="eyebrow">CARNET DE TRANSMISSIONS</p><h1>{selected?.prenom ?? 'Sélectionnez un bénéficiaire'}</h1>{selected && <p className="meta"><span className="active-pill">Actif</span> {transmissions.length} transmission{transmissions.length !== 1 ? 's' : ''}</p>}</div></div>{error && <div className="error-banner">{error}</div>}{selected ? <><form className="transmission-form" onSubmit={createTransmission}><div className="form-label"><span>Nouvelle transmission</span><small>Partagez une observation avec l’équipe</small></div><textarea name="texte" placeholder="Écrire une observation..." required /><div className="form-actions"><select name="tagId" defaultValue=""><option value="">Sans tag</option>{tags.map((tag) => <option value={tag.id} key={tag.id}>{tag.libelle}</option>)}</select><button className="primary-button" type="submit">Publier <span>↗</span></button></div></form><div className="timeline">{transmissions.map((item) => <article className="transmission" key={item.id}><div className="timeline-dot" /><div className="transmission-head"><strong>{item.auteurNom ?? 'Équipe Relais'}</strong><time>{new Date(item.creeLe).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })}</time></div><p>{item.texte}</p><div className="tag-list">{item.tags.map((tag) => <span className={tag.estAlerte ? 'tag alert' : 'tag'} key={tag.id}>{tag.libelle}</span>)}</div></article>)}{transmissions.length === 0 && <div className="empty-state"><span>✦</span><h3>Le carnet est vide</h3><p>La première observation donnera vie au suivi de {selected.prenom}.</p></div>}</div></> : <div className="empty-state large"><span>◌</span><h3>Choisissez un bénéficiaire</h3><p>Les transmissions et observations apparaîtront ici.</p></div>}</section></section>{showCreate && <div className="modal-backdrop" onClick={() => setShowCreate(false)}><div className="modal" onClick={(event) => event.stopPropagation()}><button className="modal-close" onClick={() => setShowCreate(false)}>×</button><p className="eyebrow">NOUVEAU DOSSIER</p><h2>Ajouter un bénéficiaire</h2><form className="stack-form" onSubmit={createBeneficiary}><label>Prénom<input name="prenom" placeholder="Prénom" autoFocus required /></label><button className="primary-button" type="submit">Créer le dossier <span>→</span></button></form></div></div>}</main>
}
